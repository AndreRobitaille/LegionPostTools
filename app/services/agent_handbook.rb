class AgentHandbook
  ...
  GUIDED_WORKFLOWS = [
    {
      ...
      purpose: "Place officer-supplied business on a past meeting's draft agenda, using Tracked Items only for business that should continue across meetings.",
      steps: [
        ...
        "If long-lived business has no dated row, POST its tracker to the agenda with dated_agenda_section_id so its snapshot lands in the correct section.",
        "If one-meeting business has no existing row, POST a standalone dated item into the correct section. Do not create a catalog item or tracker merely as a transport step.",
  ...
  CATALOG = [
    ...
    { name: "create_standalone_dated_agenda_item", method: "POST", path: "/api/dated_agendas/:dated_agenda_id/items", capability: "manage_agendas", group: :common,
      summary: "Create one meeting-specific item on a draft agenda without creating a catalog entry or tracker. The section id, title, and behavior_type are required; the item appends to that section.",
