module Admin
  class DatedAgendaEndeavorsController < ApplicationController
    before_action -> { require_capability("manage_agendas") }
    before_action :set_context
    before_action :ensure_draft_agenda

    def new
      existing_ids = @dated_agenda.dated_agenda_items.where.not(endeavor_id: nil).pluck(:endeavor_id).to_set
    ...
    def create
      ...
      redirect_to edit_admin_dated_agenda_path(@dated_agenda), notice: "Endeavor added."
    rescue ActiveRecord::RecordInvalid, ActiveRecord::RecordNotUnique
      redirect_to new_admin_dated_agenda_endeavor_path(@dated_agenda, dated_agenda_section_id: @agenda_section.id), alert: "That Endeavor is already on this agenda."
    end
    ...
    def redirect_locked_agenda
      redirect_to edit_admin_dated_agenda_path(@dated_agenda), alert: "Reopen this agenda before adding an Endeavor."
    end
